...
if(($_EVENT_DATA[published] && !in_array(true, $error)) || (!$_EVENT_DATA[published])) {
	...
	// remove all current schedule data
	$SchedResult = $ScheduleDbData->getEventScheduleData($_EVENT_DATA[event_id]);
	
	$sched_id = array();
	
	while($sched_data = $SchedResult->fetchRow(MDB2_FETCHMODE_ASSOC)) {
		$sched_id[] = $sched_data[schedule_id];
	}
	
	$ScheduleDbData->delete($sched_id);
	
	
	// add all new schedule data
	$schedule_id = array();      // schedule id collector
		
	$date_set = explode(";", $_EVENT_DATA[date_string]);  // parse the date string from client string
	...
	//################################ Update Event Data #########################################//
	$Event = new Event(0, $_EVENT_DATA["event_id"]);
	...
	$EventData->update($Event, $_EVENT_DATA["calendar_id"], $_EVENT_DATA["location_id"], $_EVENT_DATA["contact_id"], $schedule_id);
	...
	$AttachmentData->removeAttachments($_EVENT_DATA["event_id"]);
	...
	if($_EVENT_DATA[attachment_ids]) {
		...
		while(list($ind, $val) = each($file_array)) {
			...
			$AttachmentData->updateEvent($val, $_EVENT_DATA["event_id"]);
	...
	$redirect = "../eventdetails.php?id=" . $_EVENT_DATA["event_id"];
	
	$Smarty->assign('redirect', $redirect);
	
	$Smarty->display('page_header.tpl');

	$Smarty->display('update_event_process.tpl');
